"""
Runs the whole thing end to end: data -> features -> ML predictions ->
risk model -> optimization -> backtest -> plots. Outputs land in ./outputs/
...
def main():
    ...
    print("\nmodel diagnostics (single train/test split, just a sanity check)...")
    split_date = panel["date"].quantile(0.7)
    diag = model_diagnostics(panel, split_date)
    print(f"train R^2: {diag['train_r2']:.4f}   test R^2: {diag['test_r2']:.4f}")
    print(f"test IC (spearman): {diag['test_information_coefficient']:.4f}")
    print("top features:")
    ...
    rebalance_dates = pd.date_range(start=all_dates.iloc[300], end=all_dates.iloc[-22], freq="MS")
    ...
    print("\noptimizing most recent snapshot...")
    ...
    print("weights (max Sharpe, 25% cap):")
...
def _plot_efficient_frontier(frontier, mu, sigma, w_opt):
    ...
    ax.scatter([opt_vol * 100], [opt_ret * 100], color="#ef4444", s=100, zorder=5, label="Max Sharpe Portfolio")

    n = len(mu)
    ew = np.ones(n) / n
    ew_ret = ew @ mu.values
    ew_vol = np.sqrt(ew @ sigma.loc[mu.index, mu.index].values @ ew)
    ax.scatter([ew_vol * 100], [ew_ret * 100], color="#6b7280", s=80, zorder=5, marker="s", label="Equal-Weight")
...
def _plot_weights_over_time(result_ml):
    monthly = result_ml["weights"].resample("ME").last()
...
def _plot_feature_importance(importance):
